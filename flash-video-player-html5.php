...
Author URI: http://www.rodriev.com

Flash Video Plugin Player with html5 for Wordpress Copyright 2010  Rodrigo Violante
...
3) Includes Joshua Eldridge Wordpress Plugin
...
function FlashVideoPlayerPlugin_adminmenu() {
	add_options_page('Flash Video', 'Flash Video', '8', 'flash-video-player-html5.php', 'FlashVideoPlayerPlugin_optionspage');
}
...
function FlashVideoPlayerPlugin_renderplayer($tag_string) {	
	...
	if(strpos($inline_options['html5'], 'http://') !== false   || strpos($inline_options['html5'], 'https://') !== false) {
		// This is a remote file, so leave it alone but clean it up a little
		$find = array('&#035;', '&#036;', '&#037;', '&#038;', '&#043;', '&#044;', '&#045;', '&#063;', '&#095;', '&amp;');
		$replace = array('#', '$', '%', '&', '+', ',', '-', '?', '_', '&');
		$inline_options['html5'] = str_replace($find, $replace, $inline_options['html5']);
	} else {
		$inline_options['html5'] = $site_url . '/' . $inline_options[html5];
	}
	...
	$output .= "\n" . '<!-- Start HTML5 and Flash Video Player Plugin -->';
